...
COMMAND_HELP = {
    ...
    "block": "Block one or more tasks with a required reason",
    "unblock": "Unblock one or more tasks and return them to backlog",
    ...
    "link": "Link code/test/doc references to one or more tasks or set verification command",
    ...
    "sprint": "Manage sprint task queue (add/remove accept comma/space separated task IDs)",
...
Common Workflows:
  Create and work on a feature:
    ...
    taskpy link FEAT-01,FEAT-02 --code src/auth.py --test tests/test_auth.py
...
--------------
  ...
  taskpy list --tags backend,api        Filter by tags (comma-delimited)
...
SPRINT MANAGEMENT
-----------------
  taskpy sprint add TASK-ID[,TASK-ID]   Add task(s) to current sprint
  taskpy sprint remove TASK-ID[,TASK-ID] Remove task(s) from sprint
...
----------------
  ...
  taskpy list --format cards            Card output for dashboards
...
---------------------------------
  taskpy link TASK-ID[,TASK-ID] --code src/foo.py         Link source file(s)
  taskpy link TASK-ID[,TASK-ID] --test tests/test_foo.py  Link test file(s)
  taskpy link TASK-ID[,TASK-ID] --docs docs/design.md     Link documentation
  taskpy link TASK-ID[,TASK-ID] --verify "pytest tests/"  Set verification command
